...
const ArtistTrackLikeButton =  ({ track_info }: LikeButtonProps) => {
  ...
  const likeClick = async () => {
    try {
      const starTrackParam: STAR_TRACK_REQUEST_TYPE = {
        tracks: [{ type: 'CONCERT_HALL', clientKey: track_info.TRACK_ID }]
      };
      ...
      if (trackStarResponse.id !== null) {
        ...
        const setTrackParam: REG_TRACK_REQEUST_TYPE = {
          tracks: [{ clientKey: track_info.TRACK_ID, duration, star: 1, thumbnailUrl: track_info.THUMBNAIL, title: track_info.TITLE, type: 'CONCERT_HALL' }]
        };
        await setRegTrackAxios(setTrackParam);
        const regstarparam = { ratingInfo: { type: 'CONCERT_HALL', star: 1 }, track: { id: track_info.TRACK_ID } };
...
export default ArtistTrackLikeButton;